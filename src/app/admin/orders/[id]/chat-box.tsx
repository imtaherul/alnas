...
import { Paperclip, Send, X, FileText, FileSpreadsheet, FileImage, File, Reply } from "lucide-react"
...
function FileTypeIcon({ mimeType, className }: { mimeType: string; className?: string }) {
  ...
  if (mimeType.includes("pdf") || mimeType.includes("document") || mimeType.includes("word")) return <FileText className={className} />
...
function AttachmentPreview({ attachment }: { attachment: Attachment }) {
  ...
  return (
    ...
    >
      <FileTypeIcon mimeType={attachment.mimeType} className="h-4 w-4 shrink-0 text-gray-500" />
  ...
  return (
    <div className="group relative flex items-center gap-2 rounded-lg border border-gray-200 bg-gray-50 px-3 py-2 pr-8 text-sm">
      {isImage(file.type) ? (
        ...
        <FileTypeIcon mimeType={file.type} className="h-4 w-4 shrink-0 text-gray-500" />
      )}
      <div className="min-w-0">
        ...
        {initialMessages.length === 0 ? (
          <div className="flex items-center justify-center h-40">
            <div className="text-center">
              ...
              <p className="text-gray-300 text-xs mt-1">Click Reply to start the conversation</p>
            </div>
          </div>
        ) : (
                  ...
                  <div className={`flex flex-col ${isAdmin ? "items-end" : "items-start"} max-w-[75%]`}>
                    ...
                    >
                      ...
                      {attachments.length > 0 && (
                        <div className={`mt-3 flex flex-wrap gap-2 ${!msg.content ? "mt-0" : ""}`}>
          ...
          >
            ...
            <div className="flex-1 flex items-end gap-2">
              <textarea
                ...
                className="flex-1 resize-none rounded-lg border border-gray-300 px-3 py-2.5 text-sm focus:border-primary-500 focus:outline-none focus:ring-1 focus:ring-primary-500 min-h-[40px] max-h-[120px]"
                autoComplete="off"
              />
            </div>
